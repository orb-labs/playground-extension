import {
  ...
  Ethereum,
  RPCMethod,
} from '@rainbow-me/provider/dist/references/ethereum';
import {
  IMessenger,
...
import { toHex } from '@rainbow-me/provider/dist/utils/hex';
...
import { SolanaProvider } from '../wallet-standard/src/window';

export class RainbowProvider extends EventEmitter implements SolanaProvider {
  ...
  async connect(options?: {
    ...
    if (response?.result?.[0]) {
      this.publicKey = new PublicKey(response.result[0]);
    } else {
      ...
      const response = await this.providerRequestTransport?.send(
        {
          ...
          params: [Buffer.from(transaction.serialize())?.toString('hex')],
        },
        { id },
      );

      return { signature: response?.result[0] };
    }

    return { signature: undefined as unknown as string };
  }
  ...
  async signTransaction<T extends Transaction | VersionedTransaction>(
    ...
    if (isVersionedTransaction(transaction)) {
      ...
      if (response?.result[0]) {
        // @ts-ignore
        return VersionedTransaction.deserialize(Buffer.from(data, 'hex')) as T;
      }
    } else {
      console.error('only support versioned transaction for now');
    }
  ...
  async request({
    ...
    console.log('rainbow_prefetchDappMetadata', id, response);
